...
export function DistributorsIntelligenceHeatmap({ title, height = 600 }: DistributorsIntelligenceHeatmapProps) {
  ...
  const { regions, segments, heatmapData, allPropositions } = useMemo(() => {
    ...
    const defaultRegions = ['Northeast', 'Southeast', 'Midwest', 'Southwest', 'West']
    const defaultSegments = ['Direct Sales through OEM', 'Distributors and Wholesalers', 'Retailers', 'Funeral Service Providers']
